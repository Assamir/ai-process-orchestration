...
- \`context/foundation/\` — durable: test strategy, test plan, environments, tools, test-framework guide, framework-architecture (P3, generated), page-objects (generated PO-layer map), lessons, tech-debt tracker, repo map (test surface ↔ source).
...
`,
  },
  {
    ...
    title: "Page object conventions",
    // (R-091) Conditional: only ships for Playwright stacks (TS or Java) — the
    // page-object layer this governs is Playwright-specific. Off other stacks.
    when: { frameworks: ["playwright-ts", "playwright-java"] },
    body: `# Page object conventions
...
`,
    extended: `## Extended — worked page-object examples
...
`,
  ...
  {
    rel: "context/foundation/page-objects.md",
    body: `---
title: Page objects
version: 0.1.0
last-updated: {{LAST_UPDATED}}
owner-skill: qa-page-objects
status: seeded
---
# Page objects

> Durable. The **generated map** of the Playwright page-object layer — pages, reusable
> component/fragment objects, and the locators each drives — produced by \`qa-page-objects\`
> (phase 2) from the live UI and the frontend source. Extends, but does not own, the
> "Page objects / request builders / clients" section of \`framework-architecture.md\`
> (pillar **P3**, owned by \`qa-framework-analyze\`). Follows the \`page-object-conventions\`
> guideline. Paths are inline code, not links (\`grounding\` — confirm each before relying on it).

## Inventory
> Each screen/page and the class + file that models it.

{{PAGE_OBJECT_INVENTORY}}

## Composition
> Which reusable component/fragment objects (nav bar, data table, modal, form) each page
> composes, and where a new one belongs.

{{PAGE_OBJECT_COMPOSITION}}

## Locator map
> Per element: the chosen locator (role+name → label/text → data-testid → CSS → xpath) and
> the frontend source it was derived from, at \`file:line\`.

{{PAGE_OBJECT_LOCATOR_MAP}}

## Recommended data-testid
> Elements with no stable handle — proposed \`data-testid\` for the frontend team (frontend
> \`file:line\` + attribute name). This skill never edits the frontend; these are suggestions.
