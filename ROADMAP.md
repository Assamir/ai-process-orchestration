...
output) in **v0.32.0** (fifth epic item). The backlog below holds the remaining **R-043** epic item
(file-by-file walk) plus
...
**Epic: smart, version-aware `update` (R-038/R-039/R-040/R-041/R-042 ✅ shipped → R-043).** Extends
...
The chain has a strict dependency order (R-038 → R-039 → R-040 → R-041; R-042 depends on R-038; R-043 on
...
on `toolVersion`). The remaining item below builds on that foundation. Design decisions locked: baseline
stored as **content** (self-contained, no git dependency — ✅ done in R-039); conflict UI is an
**interactive `@clack/prompts` form** (✅ done in R-041); changelog is a **machine-computed delta** (no
hand-maintained file — ✅ done in R-042, derived from the manifest baseline vs. current templates).
