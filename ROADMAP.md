...
R-059→R-061), and the **rich, prescriptive artifact-templates epic R-062→R-068** (3-perspective `plan.md`,
detailed `cases.md`, the Markdown→Jira machine + dual-output bug report, the MCP fetch layer, the
ticket-refinement deliverable, the `test-framework.md` foundation doc, and the QA test-surface lens)
together across **v0.40.0→v0.46.0**._
...
**R-062→R-068** epic (rich, prescriptive artifact templates; see
[`docs/design/qa-artifact-templates.md`](docs/design/qa-artifact-templates.md)) is **shipped** (see the
Shipped table). The backlog still holds the unscheduled
functional-coverage candidates **R-048→R-050** (`qa-a11y`, `test-strategy` guideline,
`qa-release-readiness`), **R-053** (`qa-mobile` skill), **R-055** (`qa-security` skill), and **R-057**
(`accessibility-testing` guideline, pairs with R-048).
...
**Epic: rich, prescriptive artifact templates (R-062 → R-068 — ✅ COMPLETE, all shipped v0.40.0→v0.46.0).**
Redesigned six core templates in place into a **richer, prescriptive** shape distilled from the mature TQA
reference agents (`.external`), generalized to stack-agnostic English and the existing house style. Single
source of shape preserved: `ARTIFACTS` (`model/artifacts.ts`) for runtime artifacts, `FOUNDATION`/reference
(`model/context.ts`) for durable docs; the parity test gated every item. Two new pieces of infrastructure
landed — the shared **Markdown→Jira** machine (`model/jira.ts`, R-064) and the **MCP fetch** layer (opt-in
`xray` + `markitdown` servers + the `mcp-content-fetch` guideline, R-065). Full design + verbatim templates:
[`docs/design/qa-artifact-templates.md`](docs/design/qa-artifact-templates.md). Dependency order held:
R-064 → R-066, R-065 → R-066 (R-062/R-063/R-067/R-068 independent). **R-062 (v0.40.0)** the 3-perspective
`plan.md`; **R-063 (v0.41.0)** the detailed `cases.md`; **R-064 (v0.42.0)** the Jira machine + dual-output
bug report; **R-065 (v0.43.0)** the MCP fetch layer; **R-066 (v0.44.0)** the ticket-refinement deliverable
(`qa-ticket-review` flipped to write, new `context/refinements/` area); **R-067 (v0.45.0)** the
`test-framework.md` foundation doc; **R-068 (v0.46.0)** the QA test-surface lens. Implementation note: the
TQA-specific bits (summary prefix, required label, doc-space link) ship as angle-bracket configurable slots
in the templates, and `doctor`'s broken-link check now ignores links inside code spans so example links in
the embedded conversion table / templates don't false-positive. All seven are in the **Shipped** table.
