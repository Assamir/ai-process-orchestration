...
in **v0.29.0** (second epic item). The backlog below holds the remaining **R-040 → R-043** epic items
(3-way merge → conflict UI → changelog → file-by-file walk) plus
...
**Epic: smart, version-aware `update` (R-038/R-039 ✅ shipped → R-043).** Extends R-034's deliberately
...
dependency order (R-038 → R-039 → R-040 → R-041; R-042 depends on R-038; R-043 on R-040). **R-038
(v0.28.0)** added `toolVersion` + `scaffolded X → running Y`; **R-039 (v0.29.0)** upgraded the manifest
baseline to store the full rendered base **content** (`FileBaseline { hash, content }`) so the merge has
a self-contained base. The remaining items below build the merge on that foundation. Design decisions
locked: baseline stored as **content** (self-contained, no git dependency — ✅ done in R-039); conflict
UI is an **interactive `@clack/prompts` form**; changelog is a **machine-computed delta** (no
