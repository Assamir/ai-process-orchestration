...
(third epic item). The backlog below holds the remaining **R-041 → R-043** epic items
(interactive conflict UI → changelog → file-by-file walk) plus
...
**Epic: smart, version-aware `update` (R-038/R-039/R-040 ✅ shipped → R-043).** Extends R-034's
...
The remaining items below build the interactivity on that foundation. Design decisions locked: baseline
stored as **content** (self-contained, no git dependency — ✅ done in R-039); conflict UI is an
**interactive `@clack/prompts` form**; changelog is a **machine-computed delta** (no hand-maintained file).
