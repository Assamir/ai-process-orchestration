...
R-033 (read-before-you-write standing rule) in **v0.23.0**. Newly scoped, unscheduled work lives in the
backlog below (R-034…R-037)._
...
- **R-034 — `update` / migrate command (re-scaffolding when `core` changes).** A deterministic,
  no-LLM CLI verb (sibling to `init`/`doctor`) that diffs an already-initialized target repo against the
  current `core` templates and applies additive/updated scaffold changes **without clobbering
  filled-in placeholders or user edits** — new skills, guidelines, MCP wiring, root-config rules reach
  repos that ran an older version. Dry-run preview by default, `--write` to apply; mirrors the
  `doctor --fix` / auditskill `apply-step` dry-run/write contract. *Lands in:* `core/src/cli.ts`,
  new `core/src/update/` (or extend `doctor/`), `scaffold/`, leaf `index.ts`, tests. *Traces to:*
  PRD §8 harness-engineering; the maintenance gap that today changes in `core` never reach
  initialized repos. *Rec: ⭐ high.*
- **R-035 — `environment-management` guideline.** A `GUIDELINES` entry codifying conventions for test
  environments and secrets: env matrix (local/CI/staging), config indirection (env vars, never
  in-repo secrets — composes with the `atlassian`/browser MCP `${VAR}`/`${env:VAR}` pattern), base-URL
  /test-account/data-seed handling per environment. Mandatory ✅/❌ examples + `ENV_MGMT_PATTERNS` /
  `PROJECT_ENV_WORKFLOW` phase-2 slots (per the R-026 guideline standard); `doctor` expects the file +
  its examples. *Lands in:* `core/src/model/context.ts` (`GUIDELINES`), `doctor/index.ts`,
  `tests/scaffold.test.ts`, `tests/doctor.test.ts`, `TECH.md` §12.1, `PRD.md` §8. *Traces to:*
  PRD §8 guideline suite. *Rec: medium.*
- **R-036 — `test-data-management` guideline.** A `GUIDELINES` entry on data lifecycle policy —
  freshness, isolation between tests/runs, setup/teardown & cleanup, deterministic seeds, PII/anonymization
  — complementing the R-010 factories/fixtures (which generate data; this governs how it is managed).
  Same guideline standard (✅/❌ examples + `TEST_DATA_PATTERNS` / `PROJECT_TEST_DATA_WORKFLOW` slots);
  referenced by `qa-test-data-gen` / `qa-test-automate`; `doctor` expects file + examples. *Lands in:*
  `core/src/model/context.ts` (`GUIDELINES`), `model/skills.ts`, `doctor/index.ts`, tests, `TECH.md` §12.1,
  `PRD.md` §8. *Traces to:* PRD §8; complements R-010. *Rec: medium.*
- **R-037 — Test-framework × source-code repo map (navigation for large multi-module repos).** A
  navigational index that maps the **test surface** (where tests, configs, fixtures, page objects, helpers
  live) ↔ the **tested application's source** (modules, components, entry points) so the agent locates the
  right place fast in large, multi-module/polyglot repos instead of blind-searching. **Scope (decided):
  extend `qa-reverse-engineer` (no new skill — the map belongs to reverse-engineering and reuses its C4
  L2/L3 container/component model), generated by the product's **two-phase** flow — a deterministic
  phase-1 path inventory skeleton written to `context/foundation/repo-map.md` (modules, test dirs,
  configs; always fresh, no LLM), enriched in phase 2 by `qa-reverse-engineer` with semantic test↔source
  links and entry points.** *Lands in:* `core/src/scaffold/` + `detect/` (phase-1 inventory),
  `core/src/model/context.ts` (`FOUNDATION` `repo-map.md` template), `model/skills.ts`
  (`qa-reverse-engineer` procedure), `doctor/index.ts` (expects the file), `tests/scaffold.test.ts`,
  `TECH.md` (§12.2 / §5), `PRD.md` §5. *Traces to:* PRD §1 "give the agent a map, not a manual" + §5 test
  surface; composes with R-019/R-032 (`qa-reverse-engineer` / C4). *Rec: ⭐ high.*
