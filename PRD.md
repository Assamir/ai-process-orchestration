...
- `qa-reverse-engineer` — reverse-engineer the application source into structured project docs under `context/reference/` (business, architecture, data flow, integrations, entry points, test surface; shipped R-019). Architecture follows the **C4 model** (L1 Context → L2 Container → L3 Component → L4 Code), rendered through the Mermaid `diagram-conventions` standard (R-032).
